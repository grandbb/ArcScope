...
export async function GET(request: Request, { params }: { params: { address: string } }) {
  ...
  try { const chain = requested as ChainSlug; const portfolio = makePortfolio(params.address); const balanceHex = await alchemyRpc<string>("eth_getBalance", [params.address, "latest"], chain); const nativeBalance = hexToEther(balanceHex); const config = getChain(chain); portfolio.tokens[0] = { ...portfolio.tokens[0], name: `${config.nativeCurrency} (${config.name})`, symbol: config.nativeCurrency, balance: nativeBalance, value: nativeBalance * (chain === "arc" ? 1 : portfolio.tokens[0].price) }; portfolio.tokens = [portfolio.tokens[0]]; portfolio.tokenCount = 1; portfolio.totalValue = portfolio.tokens.reduce((sum, token) => sum + token.value, 0); portfolio.tokens = portfolio.tokens.map((token) => ({ ...token, allocation: portfolio.totalValue ? token.value / portfolio.totalValue * 100 : 0 })); return NextResponse.json({ data: portfolio }, { headers: { "Cache-Control": CACHE_HEADER } }); } catch (error) { return NextResponse.json({ data: null, error: error instanceof Error ? error.message : "Portfolio provider failed" }, { status: 502 }); }
}
