...
import { metricChange, metricNumber, periodChange, type DashboardMetrics } from "@/lib/dashboard";
...
export async function GET() {
  const filters = "?excludeTotalDataChartBreakdown=true";
  ...
  const data: DashboardMetrics = {
    ...
    dexChange24h: metricChange(dex.change_1d),
    dexChange7d: periodChange(dex.totalDataChart, 7),
    dexChange30d: periodChange(dex.totalDataChart, 30),
    dexProtocols: Array.isArray(dex.protocols) ? dex.protocols.length : null,
    fees24h: metricNumber(fees.total24h),
    feesChange24h: metricChange(fees.change_1d),
